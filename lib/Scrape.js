...
class Scrape {
  ...
  async linksFetcher (link) {
    const arrOfLinks = []
    try {
      await rp('https://en.wikipedia.org' + link, async (err, res, htm) => {
        if (!err && res.statusCode === 200) {
          const $ = cheerio.load(htm)
          $('a').each((index, link) => {
            if (!arrOfLinks.includes(link)) {
              const currentLink = $(link).attr('href')
              if (currentLink !== undefined && currentLink.includes('/wiki/') && !currentLink.includes(':')) {
      ...
      // console.log('this.wikiLink + link: ', this.wikiLink + link)
      return false
    }
  }
  ...
  async printLink () {
    try {
      this.links.forEach(async (link, i) => {
        ...
        const file = link.replace(/[/wiki/]/gm, '')
        const arrOfLinks = await this.linksFetcher(link)
